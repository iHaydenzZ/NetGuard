...
pub fn run() {
    // Set a custom panic hook for safety logging (PRD S4).
    ...
    tauri::Builder::default()
        ...
        .manage(app_state)
        .invoke_handler(tauri::generate_handler![commands::get_traffic_stats])
        .setup(move |app| {
            ...
            match capture::CaptureEngine::start_sniff(
                ...
                Ok(engine) => {
                    // Store the engine in a leaked Box so it lives for the app's lifetime.
                    // Drop will run when the process exits, releasing the WinDivert handle.
                    Box::leak(Box::new(engine));
                    tracing::info!("NetGuard monitoring started");
                }
                Err(e) => {
                    tracing::warn!(
